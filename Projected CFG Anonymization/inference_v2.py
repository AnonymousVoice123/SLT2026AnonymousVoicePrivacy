...
def convert_voice_v2(source_audio_path, target_audio_path, args):
    ...
    generator = vc_wrapper_v2.convert_voice_with_streaming(
        ...
        ar_target_audio_path=args.ar_target_audio_path,
...
def main(args):
    ...
    projection_tag = args.projection_mode
    filename = (
        f"vc_v2_{source_name}_{target_name}_"
        f"{projection_tag}k{args.smooth_kernel}_"
        f"s1{args.intelligibility_cfg_rate}_s2{args.similarity_cfg_rate}_"
        f"len{args.length_adjust}_steps{args.diffusion_steps}.wav"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--projection-mode", type=str, default="smooth",
                        choices=["global", "token", "smooth"],
                        help="Projection mode for projected CFG")
    parser.add_argument("--smooth-kernel", type=int, default=21,
                        help="Smoothing kernel size when projection-mode=smooth")
    parser.add_argument("--ar-target-audio-path", type=str, default=None,
                        help="Optional AR-only target audio path. If omitted, target is used for AR as well.")
