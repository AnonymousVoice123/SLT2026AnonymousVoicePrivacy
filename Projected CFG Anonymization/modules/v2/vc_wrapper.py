...
class VoiceConversionWrapper(torch.nn.Module):
    ...
    def load_checkpoints(
        ...
        if cfm_checkpoint_path is None:
            cfm_checkpoint_path = "./pretrained/seedvc_v2/v2/cfm_small.pth"
        
            content_extractor_wide_checkpoint_path = "./pretrained/astral/bsq2048/bsq2048_light.pth"

        else:
            print(f"Loading CFM checkpoint from {cfm_checkpoint_path}...")
        if ar_checkpoint_path is None:
            ar_checkpoint_path = "./pretrained/seedvc_v2/v2/ar_base.pth"
        else:
        ...
        # content extractor
        content_extractor_narrow_checkpoint_path = "./pretrained/astral/bsq32/bsq32_light.pth"
        ...
        # style encoder
        style_encoder_checkpoint_path = "./pretrained/campplus/campplus_cn_common.bin"
    ...
    def convert_voice_with_streaming(
            ...
            projection_mode: str = "smooth",
            smooth_kernel: int = 21,

            ar_target_audio_path=None,
    ):
        """
        ...
        Args:
            ...
            projection_mode: Projection mode for projected CFG. One of global, token, smooth, none.
            smooth_kernel: Smoothing kernel when projection_mode is smooth.
            ar_target_audio_path: Optional AR-only target audio. CFM prompt/style still use target_audio_path.
        ...
        # ============================================================
        # AR-only target audio
        # - CFM still uses target_audio_path.
        # - AR can optionally use ar_target_audio_path.
        # ============================================================
        if ar_target_audio_path is not None:
            ar_target_wave = librosa.load(ar_target_audio_path, sr=self.sr)[0]
            
            ar_target_wave = ar_target_wave[: self.sr * (self.dit_max_context_len - 5)]
        else:
            ar_target_wave = target_wave

        ar_target_wave_16k = librosa.resample(
            ar_target_wave,
            orig_sr=self.sr,
            target_sr=16000,
        )
        ar_target_wave_16k_tensor = torch.tensor(ar_target_wave_16k).unsqueeze(0).to(device)
        #==============================================================
        ...
        with torch.autocast(device_type=device.type, dtype=dtype):
            ...
            ar_target_content_indices = self._process_content_features(ar_target_wave_16k_tensor, is_narrow=False)
        ...
        if convert_style:
            with torch.autocast(device_type=device.type, dtype=dtype):
                ...
                target_narrow_indices = self._process_content_features(ar_target_wave_16k_tensor, is_narrow=True)
            src_narrow_reduced, src_narrow_len = self.duration_reduction_func(source_narrow_indices[0], 1)
            tgt_narrow_reduced, tgt_narrow_len = self.duration_reduction_func(target_narrow_indices[0], 1)
            # Process src_narrow_reduced in chunks of max 1000 tokens
            max_chunk_size = self.ar_max_content_len - tgt_narrow_len
            #max_chunk_size = self.ar_max_content_len
            ...
            for i in range(0, len(src_narrow_reduced), max_chunk_size):
                ...
                with torch.autocast(device_type=device.type, dtype=dtype):
                    ...
                    if anonymization_only:
                        ...
                        #chunk_ar_cond = self.ar_length_regulator(chunk[None])[0]
                        chunk_ar_out = self.ar.generate(chunk_ar_cond, ar_target_content_indices, compiled_decode_fn=self.compiled_decode_fn,
                                                      top_p=top_p, temperature=temperature,
                                                      repetition_penalty=repetition_penalty)
